...
from shape_prog.common.exceptions import ParseError
...
if __name__ == '__main__':
    ...
    if len(args) == 1 or args[1] not in ['-n', '-f']:
        print("Usage:\n"
              "    python3 -m shape_prog.cli -n <integer::count of elements> "
              "<str::output file> <str::output sorted file>\n"
              "or\n"
              "    python3 -m shape_prog.cli -f <str::input file> <str::output file> <str::output sorted file>\n")
        exit(5)
    elif args[1] == '-n':
        if len(args) == 5:
            cnt = args[2]
            ...
            print("In random generator mode, use\n"
                  ...
                  "<str::output file> <str::output sorted file>\n"
                  "Example:\n"
                  "    python3 -m shape_prog.cli -n 1000 outrnd_1.txt outrnd_2.txt")
            exit(3)
    elif args[1] == '-f':
        if len(args) == 5:
            in_file = args[2]
    ...
    try:
        ...
        with open(out_file, 'w') as out:
            ...
            for i, shape in enumerate(container):
                out.write(f"{i}: {shape}\n")
